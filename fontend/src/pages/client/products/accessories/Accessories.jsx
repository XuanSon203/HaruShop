...
function Accessories() {
  ...
  // Gọi API theo categoryId nếu có
  useEffect(() => {
    const fetchByCategory = async (categoryId) => {
      const res = await fetch(
        `${API_BASE}/products/accessory/category/${categoryId}`,
        {
          credentials: "include",
        }
      );
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || data.error || "Fetch accessories failed");
    ...
    const fetchAllAccessories = async () => {
      ...
      const data = await res.json();
      if (!res.ok)
        throw new Error(data.message || data.error || "Fetch all accessories failed");
      const items = Array.isArray(data.accessories)
        ? data.accessories
    ...
    const bootstrap = async () => {
      try {
        ...
        // Nếu không có categoryId, lấy tất cả phụ kiện
        await fetchAllAccessories();
      } catch (err) {
        console.error("Lỗi khi fetch phụ kiện:", err);
  ...
  // Filter và sort accessories
  useEffect(() => {
    let filtered = [...accessories];

    // Lọc bỏ sản phẩm nổi bật đã hết hàng (chỉ hiển thị sản phẩm nổi bật còn hàng)
    filtered = filtered.filter((accessory) => {
      const stock = Number(accessory?.quantity ?? 0);
      const sold = Number(accessory?.sold ?? accessory?.sold_count ?? 0);
      const remainingStock = Math.max(0, stock - sold);
      const isOutOfStock = remainingStock <= 0;
      
      // Kiểm tra sản phẩm nổi bật (hỗ trợ cả featured và is_featured)
      const isFeatured = accessory.featured || accessory.is_featured || false;
      
      // Nếu là sản phẩm nổi bật và hết hàng thì ẩn đi
      if (isFeatured && isOutOfStock) {
        return false;
      }
      return true;
    });
    ...
    if (filters.minPrice) {
      filtered = filtered.filter((accessory) => {
        const finalPrice = getFinalPrice(accessory);
    ...
    filtered.sort((a, b) => {
      // Kiểm tra sản phẩm mới (hỗ trợ cả isNew, is_New, và is_New)
      const aIsNew = a.isNew || a.is_New || false;
      const bIsNew = b.isNew || b.is_New || false;
      
      // Kiểm tra sản phẩm nổi bật (hỗ trợ cả featured và is_featured)
      const aIsFeatured = a.featured || a.is_featured || false;
      const bIsFeatured = b.featured || b.is_featured || false;
      
      // Ưu tiên sản phẩm nổi bật và mới
      const aPriority = (aIsFeatured ? 2 : 0) + (aIsNew ? 1 : 0);
      const bPriority = (bIsFeatured ? 2 : 0) + (bIsNew ? 1 : 0);
    ...
    // Reset display count khi filteredAccessories thay đổi
    // Bắt đầu với 8 sản phẩm (hoặc ít hơn nếu không đủ)
    setDisplayCount(Math.min(8, filtered.length));
  }, [accessories, filters]);
  ...
  const handleLoadMore = () => {
    setDisplayCount(prev => {
      // Tính số sản phẩm thực tế đang hiển thị (sau khi tự động fill)
      let currentDisplay = prev;
      const currentRemainder = prev % 5;
      if (currentRemainder > 0 && filteredAccessories.length > prev) {
        const neededToFill = 5 - currentRemainder;
        const availableToAdd = Math.min(neededToFill, filteredAccessories.length - prev);
        if (availableToAdd > 0) {
          currentDisplay = prev + availableToAdd;
        }
      }
      
      // Tính từ số lượng thực tế đang hiển thị để thêm sản phẩm mới
      // Nếu đã đủ 5, thêm 5 sản phẩm tiếp theo
      return currentDisplay + 5;
    });
  };
  ...
  // Tự động điều chỉnh để hàng cuối luôn đủ 5 sản phẩm nếu có thể
  let displayedAccessories = filteredAccessories.slice(0, displayCount);
  const currentRemainder = displayCount % 5;
  
  // Nếu hàng cuối chưa đủ 5 sản phẩm và còn sản phẩm để hiển thị, tự động thêm
  if (currentRemainder > 0 && filteredAccessories.length > displayCount) {
    const neededToFill = 5 - currentRemainder;
    const availableToAdd = Math.min(neededToFill, filteredAccessories.length - displayCount);
    if (availableToAdd > 0) {
      displayedAccessories = filteredAccessories.slice(0, displayCount + availableToAdd);
    }
  }
  
  // Sau khi auto-fill, kiểm tra lại xem hàng cuối có đủ 5 sản phẩm không
  // Nếu vẫn chưa đủ và còn sản phẩm, tiếp tục fill
  const finalDisplayCount = displayedAccessories.length;
  const finalRemainder = finalDisplayCount % 5;
  if (finalRemainder > 0 && finalRemainder < 5 && filteredAccessories.length > finalDisplayCount) {
    const neededToFill = 5 - finalRemainder;
    const availableToAdd = Math.min(neededToFill, filteredAccessories.length - finalDisplayCount);
    if (availableToAdd > 0) {
      displayedAccessories = filteredAccessories.slice(0, finalDisplayCount + availableToAdd);
    }
  }
  
  // Tính hasMore dựa trên displayCount thực tế, không phải displayedAccessories.length
  // Vì displayedAccessories có thể đã được auto-fill
  const hasMore = filteredAccessories.length > displayCount;
  // Chỉ hiển thị nút "Thu gọn" khi đã click "Xem thêm" (displayCount > 8)
  const canCollapse = displayCount > 8;
    ...
    <div>
      ...
      {!loading && !error && (
        <div className="mb-3">
          <small className="text-muted">
            Hiển thị {displayedAccessories.length} trong tổng số {filteredAccessories.length} sản
            phẩm {filteredAccessories.length !== accessories.length && `(từ ${accessories.length} sản phẩm)`}
          </small>
        </div>
      )}
      ...
      <Row className="g-4 products-row">
        ...
        {displayedAccessories.map((item, index) => {
          ...
          const remainingStock = stockInfo.remainingStock;
          return (
          <Col key={item._id} xs={12} sm={6} md={4} className="product-col-lg">
            <Card 
              ...
              <div style={{ position: "relative", overflow: "hidden", width: "100%", aspectRatio: "4 / 3" }}>
                ...
                {(item?.isNew || item?.is_New) && (
                  <Badge
                    className="position-absolute"
                    style={{
                      top: (item?.featured || item?.is_featured) ? 60 : 12,
                      left: 12,
                      background:
      ...
      {((hasMore || canCollapse) && !loading && !error) && (
        <div className="text-center mt-4 d-flex gap-3 justify-content-center flex-wrap">
          ...
          {hasMore && (
            ...
            >
              Xem thêm ({filteredAccessories.length - displayCount} sản phẩm)
            </Button>
          )}
        </div>
      )}
